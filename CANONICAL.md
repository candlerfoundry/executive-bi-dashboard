...
**Lineup — 17 cards across 3 rows (12 have backs, 5 are front-only) (updated 2026-06-04):**
...
**Excluded (2026-06-04):** `joel-kemp` is intentionally NOT in the lineup — the card is unapproved. Do not re-add it. (Dormant `Joel_Kemp.jpg` references in the faculty-headshot system are a different feature and do not render an impact card.)
...
**Flipbook URL (live as of 2026-06-03):** The Courses in the Community card's `lookbookUrl` and `actions[0].url` in `mission-page.json` now point to the real flipbook: `https://foundryflipbook.netlify.app/#page=1` (was the `#flipbook` placeholder). The flipbook is intentionally non-functional in mobile browsers by design, but the link is still wired on all views. No code change was required — JSON only.
